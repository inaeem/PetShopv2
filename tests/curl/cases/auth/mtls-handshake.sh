...
verbose="$OUT_GROUP/mtls.verbose.txt"
body="$OUT_GROUP/mtls.json"
set +e
...
else fail "curl exit $ec — TLS/transport failure (responses/$GROUP/mtls.verbose.txt)"; fi
...
else fail "no TLS handshake in verbose trace (responses/$GROUP/mtls.verbose.txt)"; fi
...
if echo "$trace" | grep -Eq 'SSL certificate problem|alert (bad|unknown|certificate)|handshake failure'; then
  fail "certificate rejected during handshake (responses/$GROUP/mtls.verbose.txt)"
